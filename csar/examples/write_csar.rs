...
pub fn main() {
    ...
    archive.add_string("service.yaml", content, creator.compression, None).expect("add_string");

    // Let's add an artifact
    // 10 million identical bytes should compress nicely...

    let content = vec![123; 10_000_000];
    archive.add_bytes(Path::new("artifacts").join("stuff"), &content, creator.compression, None).expect("add_bytes");
    ...
    // (you would normally be using create_for_seek_writer directly to the file)
