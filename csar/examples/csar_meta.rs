...
"#;
    ...
    let tosca_meta = ToscaMeta::from_bytes(source, &mut FailFastErrorRecipient).expect("from_bytes");
    ...
    let tosca_meta = ToscaMeta::from_string(&source, &mut FailFastErrorRecipient).expect("from_string");
