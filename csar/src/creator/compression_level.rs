...
impl CompressionLevel {
    ...
    /// Our compression is 1 to 10, Gzip is 0 to 9.
    ...
    /// Our compression is 1 to 10, ZIP DEFLATE (with flate only) is 1 to 9.
...
impl TryFrom<usize> for CompressionLevel {
    ...
    fn try_from(compression_level: usize) -> Result<Self, Self::Error> {
        if (compression_level >= 1) && (compression_level <= 10) {
            ...
            Err(CsarError::Invalid(format!("compression level must be 1 to 10: {}", compression_level)))
        }
    }
}
