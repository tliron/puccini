...
pub struct MetaBlockReader<'read, ReadT>
where
    ReadT: io::BufRead,
{
    /// Characters.
...
impl<'read, ReadT> MetaBlockReader<'read, ReadT>
where
    ReadT: io::BufRead,
{
    /// Constructor.
    ...
    /// Read block.
    pub fn read_block(&mut self) -> Result<Option<MetaBlock>, CsarError> {
        match self.chars.peek() {
            Some(c) => match c {
                ...
                Err(error) => Err(MetaError::Malformed(error.to_string()).into()),
            },
    ...
    pub fn read_key(&mut self) -> Result<Option<(String, String)>, CsarError> {
        ...
        if !keyname.is_empty() {
            if matches!(mode, KeyValueMode::Key) {
                return Err(MetaError::Malformed("keyname not followed by `:`".into()).into());
            }
    ...
    fn must_skip_space(&mut self) -> Result<(), CsarError> {
        if let Some(next) = self.chars.next() {
            let next = next?;
            if next != ' ' {
                return Err(MetaError::Malformed("`:` not followed by space".into()).into());
            }
        }
        Ok(())
    }
    ...
    fn skip_spaces(&mut self) -> Result<(), CsarError> {
        while let Some(next) = self.chars.peek() {
            match next {
                ...
                Err(error) => return Err(MetaError::Malformed(error.to_string()).into()),
            }
        }
