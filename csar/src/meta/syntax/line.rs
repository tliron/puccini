...
/// Keys to CSAR meta.
pub fn keys_to_meta<'iter, IteratorT>(iterator: IteratorT, max_columns: Option<usize>) -> String
where
    IteratorT: Iterator<Item = (&'iter String, &'iter String)>,
{
    ...
    for (key, value) in iterator {
        string += &key_to_meta(&key, &value, max_columns);
    }
...
/// Key to CSAR meta.
pub fn key_to_meta(key: &str, value: &str, max_columns: Option<usize>) -> String {
    let mut string = String::default();

    let line = format!("{}: {}", key, value);

    let line = match max_columns {
        Some(max_columns) => break_meta_line(line, max_columns),
        None => line,
    };
...
/// Break line in CSAR meta if necessary.
pub fn break_meta_line(line: String, mut max_columns: usize) -> String {
