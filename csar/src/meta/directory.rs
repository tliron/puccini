...
impl Meta {
    /// From directory.
    pub fn from_directory<ErrorRecipientT>(
        directory: &Path,
    ...
    {
        Ok(if let Some(path) = unwrap_or_give_and_return!(meta_path_in_directory(directory), errors, Ok(None)) {
            Some(Meta::read_path(&path, errors)?)
        } else {
            None
        })
    }
    ...
    pub fn validate_definitions_in_directory<ErrorRecipientT>(
        &self,
        directory: &Path,
        errors: &mut ErrorRecipientT,
    ) -> Result<(), CsarError>
        ...
        ErrorRecipientT: ErrorRecipient<CsarError>,
    {
        ...
        if let Some(entry_definitions) = &self.entry_definitions {
            ...
            if !path.exists() {
                errors.give(MetaError::FileNotFound(path.display().to_string()))?;
            }
        }
