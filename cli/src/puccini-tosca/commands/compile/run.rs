...
impl Compile {
    ...
    pub fn run(&self, root: &Root) -> Result<(), Problem> {
        ...
        if !self.instantiate {
            if !self.events.is_empty() || self.update {
                return Err(ExitError::failure("cannot use `--event` without `--instantiate`"));
            }

            if !self.inputs.is_empty() {
                return Err(ExitError::failure("cannot use `--inputs` without `--instantiate`"));
            }

            if !self.inputs_from.is_empty() {
                return Err(ExitError::failure("cannot use `--inputs-from` without `--instantiate`"));
            }

            if !self.outputs.is_empty() {
                return Err(ExitError::failure("cannot use `--output` without `--instantiate`"));
            }
        }
        ...
        AnnotatedT: 'static + Annotated + Clone + fmt::Debug + Default + Send + Sync,
    {
        ...
        if self.should_compile() {
            let mut context = CompilationContext::new(
                ...
                &catalog,
                &directory,
                store.clone().into_ref(),
                tosca_problems.as_ref(),
            );
            floria_service_template_id = if self.no_annotations {
    ...
    fn source_id(&self, url_context: &UrlContextRef, problems: &mut Problems) -> Result<SourceID, Problem> {
            ...
            && let Some(format) = Format::from_url(&url)
        {
            ...
            let entry_definitions_url = csar_url.entry_definitions_url(problems)?;
