...
Otherwise, *if a Floria store is not specified*, it will print the resulting Floria entities to stdout, again in a human-readable format. (More about this printout below.)
...
It might seem as if the output of `compile` is that human-readable printout of Floria entities, but that is not in fact the intended use. Actually, what you are seeing is *debug* output, enabled as a side effect of *not* specifying a Floria store. Otherwise, you could produce it explicitly via the `--debug=compiled` flag.

We've chosen to default to debug behavior in order to allow `compile` to be minimally usable without having to stand up a Floria store. What is happening in this case is that Puccini is storing the Floria entities in an in-memory Floria store, which is discarded as soon as the `puccini-tosca` exits. By printing out these entities we are at least producing *some* useful output.

For `compile` to be truly useful for cloud orchestration you would want the results to be stored in a running Floria store (with the `--floria` flag), or at least output a format that can be imported into Floria (with the `--format` flag).
