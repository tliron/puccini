...
impl FloriaToscaType for Vec<floria::ID> {
    fn add_tosca_type(
        ...
        let id_name = to_floria_id_name(&entity_kind_name, &type_name.to_string());
        let mut id = floria::ID::new_with_name(floria::EntityKind::Class, context.directory.clone(), id_name)?;
