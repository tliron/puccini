...
impl<AnnotatedT> ExpressionUtilities<AnnotatedT> for Option<Expression<AnnotatedT>> {
        ...
        AnnotatedT: Annotated + Clone + Default,
    {
        *self = match self.take() {
            Some(expression) => {
                if expression.is_native(function) {
                ...
                } else if other.is_native(function) {
            ...
            None => {
                if !always || other.is_native(function) {
                    // As is
                    Some(other)
                } else {
                    // Embed in function
        ...
        AnnotatedT: Annotated + Clone + Default,
    {
        if let Some(parent_validation) = parent_validation {
            self.join(parent_validation.clone(), "and", floria::CallKind::Normal, false);
