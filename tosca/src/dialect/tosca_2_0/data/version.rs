...
impl<AnnotatedT> Into<Variant<AnnotatedT>> for Version
where
    AnnotatedT: Default,
{
    fn into(self) -> Variant<AnnotatedT> {
        ...
        map.into_insert("major", self.major as i64);
        map.into_insert("minor", self.minor as i64);

        if let Some(fix) = self.fix {
            map.into_insert("fix", fix as i64);
        }

        if let Some(qualifier) = self.qualifier {
            map.into_insert("qualifier", qualifier);
        }

        if let Some(build) = self.build {
            map.into_insert("build", build as i64);
        }
