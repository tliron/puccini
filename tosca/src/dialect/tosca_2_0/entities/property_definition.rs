    ...
    AnnotatedT: Annotated + Clone + Default,
{
    ...
    ///
    /// Used for complex data type property definitions
    ...
    {
        if let Ok(data_type) =
            catalog.get_entity::<DataType<AnnotatedT>, AnnotatedT>(DATA_TYPE, &self.type_name, source_id)
        {
            // TODO: our validation?
            if let Some(reference) = data_type.initialize_schema(schema, self, source_id, catalog)? {
                return Ok(Some(StructSchemaField::new(reference, self.required.unwrap_or(true))));
            }
        }
