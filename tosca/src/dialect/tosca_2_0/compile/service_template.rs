...
impl Dialect2_0 {
        ...
        AnnotatedT: 'static + Annotated + Clone + Default,
    {
        ...
        get_or_create_native_plugin(context)?;
    ...
    AnnotatedT: 'static + Annotated + Clone + Default,
{
    ...
    pub fn compile(
        ...
        if let Some(plugin_id) = get_or_create_native_plugin(context)?
            && let Some(function_name) =
                ok_give!(floria::FunctionName::new(plugin_id, ByteString::from_static("_set_inputs")), context.errors)
        {
            vertex_template.template.add_event_handler(floria::ADDED_EVENT, function_name);
        }
