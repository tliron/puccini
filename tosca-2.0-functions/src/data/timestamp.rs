...
impl TryFrom<Expression> for Timestamp {
    ...
    fn try_from(expression: Expression) -> Result<Self, Self::Error> {
        match expression {
            ...
            _ => Err(format!("timestamp not a |name|string| or |name|custom|: |error|{}|", expression.type_name())),
        }
    }
}
...
impl TryFrom<&Custom> for Timestamp {
    ...
    fn try_from(custom: &Custom) -> Result<Self, Self::Error> {
        ...
        let Expression::Map(map) = &custom.inner else {
            return Err(format!("timestamp not a |name|map|: |error|{}|", custom.inner.type_name()));
        };
...
fn get_integer(map: &BTreeMap<Expression, Expression>, name: &'static str) -> Result<i32, String> {
    match map.get(&name.into()) {
        Some(Expression::Integer(integer)) => {
            ...
            Ok(result.map_escape_depiction_error()?)
        }

        Some(Expression::UnsignedInteger(unsigned_integer)) => {
            let result: Result<_, TryFromIntError> = (*unsigned_integer).try_into();
            Ok(result.map_escape_depiction_error()?)
        }

        Some(expression) => Err(format!(
            "timestamp |meta|{}| key not an |name|integer| or an |name|unsigned integer|: |error|{}|",
            name,
