...
pub trait ExpressionUtilities: Sized {
    ...
    // /// Coerce into a data kind.
    // fn coerce(self, data_kind: &str) -> Result<Option<Expression>, String> {
    //     let (expression, _modified) = self.maybe_coerce(data_kind)?;
    //     Ok(Some(expression))
    // }
...
impl ExpressionUtilities for Expression {
    fn maybe_coerce(self, data_kind: &str) -> Result<(Expression, bool), String> {
        ...
        let article = match data_kind {
            INTEGER_DATA_KIND => "an",
            _ => "a",
        };

        Err(format!("not {} |name|{}|: |error|{}|", article, data_kind, self.type_name()))
    }
    ...
    fn coerce_if_custom(self, other: &Expression) -> Result<Expression, String> {
            ...
            && !matches!(self, Expression::Custom(_))
        {
            ...
            match custom.kind.as_str() {
                ...
                kind => Err(format!("unsupported |name|custom| kind: |error|{}|", escape_depiction_markup(kind))),
            }
        } else {
            Ok(self)
        }
        ...
        match self {
            ...
            Expression::Custom(custom) => {
                ...
                match custom.kind.as_str() {
                    ...
                    kind => Err(format!("unsupported custom kind: |error|{}|", escape_depiction_markup(kind))),
                }
            }

            _ => Err(format!("cannot compare |error|{}|", self.type_name())),
        }
    }
}
