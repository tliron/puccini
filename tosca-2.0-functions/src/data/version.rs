...
impl TryFrom<Expression> for Version {
    ...
    fn try_from(expression: Expression) -> Result<Self, Self::Error> {
        match expression {
            ...
            _ => Err(format!("version not a |name|string| or a |name|custom|: |error|{}|", expression.type_name())),
        }
    }
}
...
impl TryFrom<&Custom> for Version {
    ...
    fn try_from(custom: &Custom) -> Result<Self, Self::Error> {
        ...
        let Expression::Map(map) = &custom.inner else {
            return Err(format!("version not a |name|map|: |error|{}|", custom.inner.type_name()));
        };
...
fn get_unsigned_integer_option(
    ...
    match map.get(&name.into()) {
        Some(Expression::Integer(integer)) => {
            let result: Result<_, TryFromIntError> = (*integer).try_into();
            Ok(Some(result.map_escape_depiction_error()?))
        }

        Some(Expression::UnsignedInteger(unsigned_integer)) => Ok(Some(*unsigned_integer)),

        Some(expression) => Err(format!(
            "version |meta|{}| key not an |name|integer| or an |name|unsigned integer|: |error|{}|",
            name,
...
fn get_string_option(map: &BTreeMap<Expression, Expression>, name: &'static str) -> Result<Option<String>, String> {
    match map.get(&name.into()) {
        ...
        Some(expression) => {
            Err(format!("version |meta|{}| key not a |name|string|: |error|{}|", name, expression.type_name()))
        }
        None => Ok(None),
    }
}
