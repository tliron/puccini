...
impl Scalar {
    ...
    pub fn new_from_expression(expression: Expression, schema: &ScalarSchema) -> Result<Self, String> {
        match expression {
            ...
            _ => Err(format!("scalar not a |name|string| or |name|custom|: |error|{}|", expression.type_name())),
        }
    }
    ...
    pub fn canonical(&self) -> Result<Number, String> {
        if self.is_canonical() { Ok(self.number) } else { self.number.multiply(self.schema.canonical_factor) }
    }
}
...
impl TryFrom<Expression> for Scalar {
    ...
    fn try_from(expression: Expression) -> Result<Self, Self::Error> {
        match expression {
            Expression::Custom(custom_resource) => custom_resource.custom().try_into(),
            _ => Err(format!("scalar not a |name|custom|: |error|{}|", expression.type_name())),
        }
    }
}
...
impl TryFrom<&Custom> for Scalar {
    ...
    fn try_from(custom: &Custom) -> Result<Self, Self::Error> {
        ...
        let Expression::Map(map) = &custom.inner else {
            return Err(format!("scalar not a |name|map|: |error|{}|", custom.inner.type_name()));
        };
        ...
        let Expression::Text(unit) = unit else {
            return Err("scalar |meta|unit| key not a string".into());
        };
