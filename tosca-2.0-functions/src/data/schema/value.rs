...
impl ValueSchema {
    ...
    pub fn coerce(&self, expression: Expression, schema: &Schema, call_site: &CallSite) -> Result<Expression, String> {
        match self {
            Self::Reference(reference) => schema.get(*reference)?.coerce(expression, schema, call_site),
    ...
    pub fn coerce_option(
        ...
        match self {
            Self::Reference(reference) => schema.get(*reference)?.coerce_option(expression, schema, call_site),
...
impl TryFrom<Expression> for ValueSchema {
    ...
    fn try_from(expression: Expression) -> Result<Self, Self::Error> {
        ...
        match expression {
            ...
            _ => Err(format!(
                "value schema not a |name|map|, |name|string|, or |name|unsigned integer|: |error|{}|",
