    ...
    fn try_from(expression: Expression) -> Result<Self, Self::Error> {
        match expression {
            Expression::Map(map_resource) => {
                ...
                let (canonical_unit, canonical_factor) =
                    find_canonical_unit_and_factor(canonical_unit, &units, &prefixes)?;
                Ok(Self::new(data_kind, units, canonical_unit, canonical_factor, prefixes, default, validation))
            }

            _ => Err(format!("scalar schema not a |name|map|: |error|{}|", expression.type_name())),
        }
    }
}
...
fn get_string_option(map: &Map, name: &'static str) -> Result<Option<String>, String> {
    match map.into_get(name) {
        ...
        Some(value) => {
            Err(format!("scalar schema |meta|{}| key not a |name|string|: |error|{}|", name, value.type_name()))
        }
        None => Ok(None),
    }
}
...
fn get_map(map: &Map, name: &'static str) -> Result<BTreeMap<String, Number>, String> {
    match map.into_get(name) {
        Some(Expression::Map(map)) => {
            ...
            for (key, value) in &map.map().inner {
                let Expression::Text(key) = key else {
                    return Err(format!(
                        "scalar schema |meta|{}| key has non-string |name|map| key: |error|{}|",
                        name,
                        key.type_name()
                    ));
                };
        ...
        Some(value) => {
            return Err(format!("scalar schema |meta|{}| key not a |name|map|: |error|{}|", name, value.type_name()));
        }
